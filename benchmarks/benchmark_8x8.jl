...
function solve()
    ...
    for (piece, piece_colors) in enumerate(eachrow(pieces)), rotation = 0:3
        ...
        if top == left == 12  # top-left corner
            left = 13
    ...
    board = FixedSizeVector{RotatedPiece}(undef, 72)
    idx_state = FixedSizeVector{Int}(undef, 72)
    ...
    t1 = time_ns()
    display(transpose(reshape(Int.(getfield.(board[9:72], :number)), 8, 8))[8:-1:1, :])
